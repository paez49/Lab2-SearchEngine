...
def find_similar_courses(words: List[str], df: pd.DataFrame) -> List[str]:
    ...
    """
    words_set = set(words)
    courses = df.groupby("courseID")["word"].apply(set).reset_index()
    ...
    return similar_courses["courseID"].tolist()


def search(
    keywords: List[str], df_index: pd.DataFrame, dict_courses: Dict[str, str]
) -> List[str]:
    """This function searches for the courses most similar to a provided list of words.
    Similarity is calculated using the Jaccard similarity metric.
        ...
        Args:
            ...
            dict_courses (Dict[str,str]): A dictionary with the courseID as key and the url as value.
